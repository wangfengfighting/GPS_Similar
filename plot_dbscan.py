...
def dbscan(EPS,MIN_SAMPLE):
    ...
    for k, col in zip(unique_labels, colors):
        ...
        xx = XX[class_member_mask & core_samples_mask]
    ...
    #--------------------------------------------------------------------------------------------------------------------------------------------#
        #plot 2chen
    YY=np.loadtxt(".\\GPS_Get_PreProcesser\\7-12-2015\\locationGPS.txt",dtype=float,delimiter=',',skiprows=1,usecols=(0,1),unpack=False)
    #Latitude,Lat,XX=GPS_Kalman_Filter.Get_Prime_GpsData(".\\GPS_Get_PreProcesser\\7-11-2015\\locationGPS.txt")
    centers = [[1, 1], [-1, -1], [1, -1]]
    db = DBSCAN(eps=EPS, min_samples=MIN_SAMPLE).fit(YY)
    #db = DBSCAN(eps=0.002, min_samples=10).fit(XX)  3
    #db = DBSCAN(eps=0.0032, min_samples=10).fit(XX)   2
    core_samples_mask = np.zeros_like(db.labels_, dtype=bool)
    core_samples_mask[db.core_sample_indices_] = True
    ...
    for k, col in zip(unique_labels, colors):
        ...
        yy = YY[class_member_mask & core_samples_mask]

        #ax.scatter(xy[:, 0], xy[:, 1],np.array([1]*len(xy[:, 0])), 'o', markerfacecolor=col,
             #markeredgecolor='k', markersize=8)
        ax.scatter(yy[:, 0], yy[:, 1],np.array([2]*len(yy[:, 0])), c=col)
